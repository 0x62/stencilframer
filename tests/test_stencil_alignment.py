...
OPEN_OUTLINE_WITH_SMALL_CLOSED_LOOPS = """
%FSLAX33Y33*%
%MOMM*%
%ADD10C,0.000*%
G54D10*
...
class StencilAlignmentTest(unittest.TestCase):
    ...
    def test_prefers_large_open_outline_over_small_closed_loops(self):
        path = write_temp(OPEN_OUTLINE_WITH_SMALL_CLOSED_LOOPS, ".gbr")
        try:
            stencil = stencilframer.parse_gerber_stencil(path)
        finally:
            os.unlink(path)
    ...
    def test_matches_translation_with_gerber_y_flip(self):
        ...
        self.assertAlmostEqual(match['angle_rad'], 0)
    ...
    def test_matches_rotation(self):
        ...
        match = stencilframer.match_stencil_to_pcb_pads(stencil_points, pcb_points)
        aligned = stencilframer.transform_stencil_points(
            stencil_points,
            flip_y=match['flip_y'],
            angle_rad=match['angle_rad'],
            offset=match['offset'],
        )

        self.assertLessEqual(match['residual'], 0.25)
        self.assertEqual(len(aligned), len(pcb_points))
        for point in aligned:
            self.assertTrue(any(stencilframer.distance(point, pcb_point) < 1e-6 for pcb_point in pcb_points))

    def test_rotates_stencil_outline_with_alignment(self):
        outline = [(0, 0), (10, 0), (10, 4), (0, 4)]
        angle = 90.0/180.0*3.141592653589793
        aligned = stencilframer.aligned_stencil_polygon(outline, {
            'flip_y': False,
            'angle_rad': angle,
            'offset': (5, 7),
        })

        self.assertAlmostEqual(aligned[0][0], 5)
        self.assertAlmostEqual(aligned[0][1], 7)
        self.assertAlmostEqual(aligned[1][0], 5)
        self.assertAlmostEqual(aligned[1][1], 17)
        self.assertAlmostEqual(aligned[2][0], 1)
        self.assertAlmostEqual(aligned[2][1], 17)
